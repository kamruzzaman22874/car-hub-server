...
async function run() {
  try {
    ...
    const brandsCollection = await client.db("carHub").collection("brands")
    const deaclersCollection = await client.db("carHub").collection("deaclare")


    app.get("/category", async (req, res) => {
      const result = await gategoryCollection.find().toArray();
      res.send(result);
    })
    ...
    app.get("/declare", async (req, res) => {
      const result = await deaclersCollection.find().toArray();
      res.send(result);
    })
